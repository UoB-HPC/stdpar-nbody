...
auto build_plummer_model(Arguments arguments) -> System<T, N> {
  ...
  for (size_t _ = 0; _ < number_of_particles; _++) {
    ...
    auto pos
     = radius
       * vec<T, N>{{std::sin(p_theta) * std::cos(p_phi), std::sin(p_theta) * std::sin(p_phi), std::cos(p_theta)}};
    ...
    auto velocity
     = velocity_norm
       * vec<T, N>{{std::sin(v_theta) * std::cos(v_phi), std::sin(v_theta) * std::sin(v_phi), std::cos(v_theta)}};
...
auto rotate_vec(std::array<std::array<T, 3>, 3> const & matrix, vec<T, 3> v) -> vec<T, 3> {
  ...
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j) result[i] += matrix[i][j] * v[j];
  return result;
}

template <typename T, dim_t N>
auto create_circular_orbit(System<T, N>& system, std::size_t number_of_particles, T total_mass, T orbit_mass,
                           vec<T, N> centre_pos) {
  for (size_t _ = 0; _ < number_of_particles; _++) {
    ...
    auto pos      = radius * vec<T, N>{{std::sin(angle), std::cos(angle)}};
    ...
    auto velocity = velocity_norm / (l2norm(pos) + std::numeric_limits<T>::epsilon()) * vec<T, N>{{-pos[1], pos[0]}};
    ...
    if constexpr (N == 3) {
      ...
      std::array<std::array<T, 3>, 3> const matrix = {{{0.0, -1.0, 0.0}, {0.9, 0.0, 0.5}, {0.5, 0.0, 0.9}}};
...
auto build_galaxy_model(Arguments arguments) -> System<T, N> {
  ...
  create_circular_orbit<T, N>(system, galaxy_number_of_particles - 1, centre_mass + 1, 1, offset_pos);
