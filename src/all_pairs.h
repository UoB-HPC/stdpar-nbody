...
void run_all_pairs_step(System<T, N>& system, Arguments arguments) {
  ...
  for (size_t step = 0; step < arguments.steps; step++) {
    ...
    std::for_each(std::execution::par_unseq, r.begin(), r.end(), [s = system.state()](auto i) {
...
void run_all_pairs_collapsed_step(System<T, N>& system, Arguments arguments) {
  ...
  for (size_t step = 0; step < arguments.steps; step++) {
    ...
    std::for_each_n(std::execution::par_unseq, it, system.size * system.size, [s = system.state()](auto p) {
      auto j = p / s.sz;
      auto i = p % s.sz;
      if (i == j) {
